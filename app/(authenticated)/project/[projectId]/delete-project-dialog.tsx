...
export default function DeleteProjectDialog({ open, project, setOpen }: Props) {
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      const response = await deleteUserProject(project.id);
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Error occured while deleting project.";
  ...
  return (
    ...
    >
      <div className="mb-4">
        <h1 className="text-2xl font-semibold">Delete “{project.name}”?</h1>
        <p className="mt-1 text-sm font-light">
          This will permanently delete this project and all of its tasks.
        </p>
        <p className="mt-1 text-sm font-light">
